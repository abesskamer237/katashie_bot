...
if [ "$EUID" -ne 0 ]; then
  ...
  echo "Exemple: sudo bash scripts/deploy-vps.sh katashie.pagnol.xyz"
  exit 1
fi
...
if [ ! -d "$APP_DIR/.git" ]; then
  rm -rf "$APP_DIR"
  git clone https://github.com/abesskamer237/katashie_bot.git "$APP_DIR"
else
  cd "$APP_DIR"
  git pull --ff-only
fi
